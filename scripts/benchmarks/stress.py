...
def sum_log_metrics(log_text: str) -> dict[str, int]:
    ...
    for raw in log_text.splitlines():
        ...
        if match := base.STRUCTURED_DEDUP_RE.search(line):
            metrics["existing_chunks"] += int(match.group(1))
            metrics["total_chunks"] += int(match.group(2))
        if "Updated global index for" in line or 'operation="job.global_index_updated"' in line:
...
class Harness:
    ...
    def start(self) -> None:
        ...
        try:
            cert_path = base.agent_certificate_path()

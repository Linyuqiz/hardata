...
use hardata_shared::error::HarDataError;
...
impl ISyncMessage {
    ...
    pub fn validate_payload_len(payload_len: u32) -> Result<usize> {
        ...
        if payload_len > hardata_shared::constants::MAX_PROTOCOL_PAYLOAD_SIZE {
            return Err(HarDataError::InvalidProtocol(format!(
                "Payload size {} exceeds maximum allowed size {}",
                payload_len,
                hardata_shared::constants::MAX_PROTOCOL_PAYLOAD_SIZE
            )));
        }
...
mod tests {
    ...
    use hardata_shared::constants::MAX_PROTOCOL_PAYLOAD_SIZE;

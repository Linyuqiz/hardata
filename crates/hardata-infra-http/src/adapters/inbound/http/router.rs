...
use hardata_app::application::use_cases::JobUseCases;
use std::sync::Arc;
...
mod tests {
    ...
    use hardata_app::application::sync::engine::scheduler::SchedulerConfig;
    ...
    async fn build_app(root: &std::path::Path, api_token: Option<String>) -> axum::Router {
        ...
        let scheduler =
            hardata_app::application::sync::engine::scheduler::SyncScheduler::new(config, db)
                .await
                .unwrap();
        create_sync_router(
            scheduler,
    ...
    async fn router_restricts_cors_headers_with_token() {
        ...
        // CORS exposes either `*` or the configured header list.

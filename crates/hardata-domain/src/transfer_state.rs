...
fn uses_legacy_second_precision(timestamp: i64) -> bool {
    timestamp.saturating_abs() < LEGACY_TIMESTAMP_THRESHOLD
}

fn timestamps_match(left: i64, right: i64) -> bool {
    if left == right {
        return true;
    }

    if uses_legacy_second_precision(left) || uses_legacy_second_precision(right) {
        let left_seconds = if uses_legacy_second_precision(left) {
            left
        } else {
            left.div_euclid(NANOS_PER_SEC)
        };
        let right_seconds = if uses_legacy_second_precision(right) {
            right
        } else {
            right.div_euclid(NANOS_PER_SEC)
        };
...
fn optional_timestamps_match(left: Option<i64>, right: Option<i64>) -> bool {
    match (left, right) {

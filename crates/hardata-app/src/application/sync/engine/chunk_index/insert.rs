...
use crate::shared::time::timestamps_match;
...
fn collect_weak_hashes(chunks: &[crate::application::sync::engine::ChunkInfo]) -> Vec<u64> {
...
fn collect_strong_hashes(chunks: &[crate::application::sync::engine::ChunkInfo]) -> Vec<[u8; 32]> {
...
pub(super) fn remove_file_locations_by_hashes(
    ...
    if removed > 0 {
        info!(
            operation = "index.reindex_cleanup_completed",
            path = %file_path,
            removed_locations = removed,
            "stale chunk locations removed before reindex"
        );
    }
...
mod tests {
    ...
    use crate::application::sync::engine::chunk_index::ChunkLocation;

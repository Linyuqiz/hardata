...
pub fn remove_file(
    ...
    info!(
        operation = "index.file_locations_removed",
        path = %file_path,
        removed_locations,
        removed_weak_hashes,
        removed_strong_hashes,
        "file chunk index locations removed"
    );
...
pub fn cleanup_stale_file_scans(
    ...
    if removed_locations > 0 || removed_file_scans > 0 {
        info!(
            operation = "index.file_scan_cleanup_completed",
            removed_locations, removed_file_scans, "stale file scan entries removed"
        );
    }
...
pub fn cleanup_stale_entries(
    ...
    if removed_locations > 0 || removed_file_scans > 0 {
        info!(
            operation = "index.cleanup_completed",
            removed_locations,
            removed_weak_hashes,
            removed_strong_hashes,
            removed_file_scans,
            "stale chunk index entries removed"
        );
    }
...
mod tests {
    ...
    use crate::application::sync::engine::chunk_index::{ChunkLocation, FileScanMetadata};

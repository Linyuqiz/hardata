...
use crate::domain::ChunkLocation as ProtocolChunkLocation;
        ...
        .await
    {
        Ok(response) => {
            ...
            if response.hashes.len() != chunks_to_verify.len() {
                warn!(
                    operation = "dedup.remote_hash_count_mismatch",
                    path = %remote_file_path,
                    received = response.hashes.len(),
                    expected = chunks_to_verify.len(),
                    "remote strong hash count mismatch"
                );
                return Ok(());
            }
        ...
        Err(e) => {
            warn!(
                operation = "dedup.remote_strong_hash_failed",
                path = %remote_file_path,
                error = %e,
                "remote strong hash verification failed"
            );
        }
    }

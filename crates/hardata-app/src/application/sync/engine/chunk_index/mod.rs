...
impl ChunkIndex {
    pub fn new(cache_path: &Path) -> Result<Self> {
        ...
        info!(operation = "index.opened", path = %cache_path.display(), "chunk index opened");
        debug!(
            operation = "index.tree_sizes",
            weak_hash_entries = weak_hash_tree.len(),
            strong_hash_entries = strong_hash_tree.len(),
            file_scan_entries = file_scans_tree.len(),
            "chunk index tree sizes loaded"
        );
    ...
    pub async fn query_chunks(
        &self,
        chunks: &mut [crate::application::sync::engine::core::FileChunk],
        dest_path: &str,
        existing_strong_hashes: &HashSet<[u8; 32]>,
        connection: &mut crate::adapters::outbound::transport::gateway::TransportConnection,
        file: &crate::application::sync::scanner::ScannedFile,
    ) -> Result<DedupResult> {
        query::query_chunks(
            &self.weak_hash_tree,
            &self.hit_count,
    ...
    pub fn batch_insert_chunks(
        &self,
        file_path: &str,
        chunks: &[crate::application::sync::engine::ChunkInfo],
        mtime: i64,
        file_size: u64,
    ) -> Result<usize> {
...
mod tests {
    ...
    use crate::application::sync::engine::ChunkInfo;
    use std::fs;

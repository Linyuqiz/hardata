use crate::adapters::outbound::transport::eyeballs::{
    ConnectionResult, HappyEyeballs, TransportProtocol,
};
use crate::adapters::outbound::transport::gateway::{
    Protocol, ProtocolSelector, TransportConnection,
};
use crate::adapters::outbound::transport::health::{HealthChecker, HealthStatus};
use crate::application::config::RegionConfig;
...
use crate::shared::constants::{MAX_RECONNECT_ATTEMPTS, RECONNECT_DELAY_MS, SCHEDULER_POOL_SIZE};
...
fn retry_config_for_mode(mode: ConnectionEstablishMode) -> crate::shared::retry::RetryConfig {
    match mode {
        ConnectionEstablishMode::Startup => crate::shared::retry::RetryConfig {
            max_retries: 1,
            initial_delay_ms: 0,
            max_delay_ms: 0,
            backoff_multiplier: 1.0,
        },
        ConnectionEstablishMode::RuntimeReconnect => crate::shared::retry::RetryConfig::default(),
    }
}
...
pub async fn establish_all_connections(
    ...
    info!(
        operation = "scheduler.connections_starting",
        region_count = config.regions.len(),
        "region connections starting"
    );
    ...
    for region in &config.regions {
        if let Some(pool) = connection_pools.get(&region.name) {
            ...
            .await
            {
                ...
                Err(err) => {
                    warn!(
                        operation = "scheduler.startup_connection_deferred",
                        region = %region.name,
                        error = %err,
                        "startup connection deferred to reconnect"
                    );
    ...
    if failures.is_empty() {
        info!(
            operation = "scheduler.connections_ready",
            region_count = config.regions.len(),
            "all region connections established"
        );
        Ok(())
    } else {
...
async fn establish_region_connection_with_mode(
    ...
    let (quic_client, tcp_client) = {
        ...
        // Rebuild QUIC when certificates appear after startup.
        if pool.quic_client.is_none() {
            if let Some(quic_host) = quic_server_name(&region.quic_bind) {
                ...
                if std::path::Path::new(&ca_cert_path).exists() {
                    match crate::adapters::outbound::transport::quic::QuicClient::new(
                        region.quic_bind.clone(),
                        quic_host.clone(),
                        ca_cert_path,
                    ) {
                        Ok(client) => {
                            info!(operation = "scheduler.quic_client_recovered", region = %region.name, "QUIC client recovered");
    ...
    match connection_result {
        ConnectionResult::Quic(conn) => {
            info!(operation = "scheduler.region_connected", region = %region.name, protocol = "quic", "region connection established");
            let mut pool = connection_pool.lock().await;
        ...
        ConnectionResult::Tcp(_stream) => {
            info!(operation = "scheduler.region_connected", region = %region.name, protocol = "tcp", "region connection established");
            let mut pool = connection_pool.lock().await;
            ...
            if pool.tcp_client.is_none() {
                ...
                match crate::adapters::outbound::transport::tcp::TcpClient::with_pool(
                    region.tcp_bind.clone(),
                    Some(SCHEDULER_POOL_SIZE),
                ) {
                    Ok(client) => {
                        pool.tcp_client = Some(client);
                        info!(operation = "scheduler.tcp_client_ready", region = %region.name, "TCP client ready");
                    }
                    Err(e) => {
                        warn!(
                            ...
                            error = %e,
                            fallback = "direct_tcp",
                            "pooled TCP client unavailable; using direct client"
                        );
                        if let Ok(simple_client) =
                            crate::adapters::outbound::transport::tcp::TcpClient::new(
                                region.tcp_bind.clone(),
                            )
                        {
    ...
    info!(operation = "scheduler.region_ready", region = %region.name, "region connection ready");
    Ok(())
}
...
pub async fn get_connection_with_retry_for_region_with_selector(
    ...
    for attempt in 0..MAX_RECONNECT_ATTEMPTS {
        match get_transport_connection(&connection_pool).await {
            ...
            Err(e) => {
                warn!(
                    operation = "scheduler.connection_attempt_failed",
                    region = %region,
                    attempt = attempt + 1,
                    error = %e,
                    "region connection attempt failed"
                );
                last_error = Some(e);
                ...
                {
                    warn!(
                        operation = "scheduler.reconnect_attempt_failed",
                        region = %region,
                        attempt = attempt + 1,
                        error = %reconnect_err,
...
async fn try_reconnect_region(
    ...
    info!(operation = "scheduler.region_reconnected", region = %region.name, "region reconnection successful");
    Ok(())
}

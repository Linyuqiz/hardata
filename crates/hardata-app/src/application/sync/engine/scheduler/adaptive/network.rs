use crate::adapters::outbound::transport::bandwidth::{BandwidthProbe, NetworkQuality};
use crate::shared::constants::EWMA_BANDWIDTH_ALPHA;
...
impl AdaptiveController {
    pub fn new(config: AdaptiveConfig) -> Arc<Self> {
        ...
        info!(
            operation = "scheduler.network_adaptive_created",
            initial,
            min_concurrency = config.min_concurrency,
            max_concurrency = config.max_concurrency,
            "network adaptive controller created"
        );
    ...
    pub async fn adjust(&self) {
        ...
        let new_concurrency = if error_rate > self.config.target_error_rate {
            ...
            info!(
                operation = "scheduler.network_concurrency_reduced",
                error_rate = error_rate * 100.0,
                previous = current,
                current = new,
                "network adaptive concurrency reduced"
            );
            new
...
impl NetworkAdaptiveController {
    pub fn new(bandwidth_probe: Arc<BandwidthProbe>) -> Arc<Self> {
        info!(
            operation = "scheduler.network_controller_created",
            "network adaptive controller created"
        );

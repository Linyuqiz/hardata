...
use crate::application::sync::engine::{copy_chunk_from_file, ChunkLocation};
...
pub async fn relocate_local_chunks(
    ...
    cancel_callback: &crate::application::sync::transfer::batch::CancelCallback,
) -> Result<u64> {
...
pub async fn copy_cross_file_chunks(
    ...
    for (chunk_idx, location, dest_offset) in chunks_to_copy {
        ...
        let Some(expected_hash) = chunk.chunk_hash.strong else {
            warn!(
                operation = "dedup.cross_file_copy_skipped",
                chunk_index = *chunk_idx,
                reason = "strong_hash_missing",
                "cross-file copy skipped"
            );
    ...
    for (chunk_idx, source, dest_offset, expected_hash) in copy_tasks {
        ...
        match copy_chunk_from_file(&source, dest_path, dest_offset, expected_hash).await? {
            crate::application::sync::engine::CopyResult::Success(bytes) => {
                success_count += 1;
                bytes_copied += bytes;
                state.mark_chunk_completed(chunk_idx);
            }
            crate::application::sync::engine::CopyResult::SourceNotFound
            | crate::application::sync::engine::CopyResult::SourceModified
            | crate::application::sync::engine::CopyResult::HashMismatch => {
                verification_failed += 1;
                failed_chunks.push(chunk_idx);
            }
            crate::application::sync::engine::CopyResult::IOError(e) => {
                warn!(
                    operation = "dedup.cross_file_copy_failed",
                    chunk_index = chunk_idx,
                    error = %e,
                    "cross-file chunk copy failed"
                );
                io_failed += 1;
                failed_chunks.push(chunk_idx);
            }
...
mod tests {
    ...
    async fn missing_strong_hash_falls_back_without_marking_completed() {
        ...
        let (bytes_copied, failed_chunks) = copy_cross_file_chunks(
            ...
            &(std::sync::Arc::new(|| false)
                as crate::application::sync::transfer::batch::CancelCallback),
        )
        .await
        .unwrap();
    ...
    async fn copy_cross_file_chunks_stops_when_cancelled() {
        ...
        let mtime = crate::shared::time::metadata_mtime_nanos(&metadata);

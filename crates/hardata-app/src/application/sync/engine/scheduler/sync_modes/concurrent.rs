...
use crate::shared::constants::SCHEDULER_POOL_SIZE;
...
pub async fn sync_files_concurrent(
    ...
    info!(
        operation = "job.concurrent_sync_started",
        job_id = %job.job_id,
        protocol = protocol_label(is_quic),
        file_count = files.len(),
        file_concurrency = actual_concurrency,
        per_file_streams,
        "concurrent file synchronization started"
    );
        ...
        futures.push(async move {
            ...
            match sync_single_file(
                ...
                move |delta| {
                    ...
                    if progress > last_progress {
                        ...
                        info!(operation = "job.transfer_progress", job_id = %job_id_copy, progress, "job transfer progress");
                    }
            ...
            .await
            {
                ...
                Err(e) => {
                    ...
                    match category {
                        ErrorCategory::Fatal => {
                            error!(operation = "job.file_sync_failed", job_id = %job_id, path = %file_name, category = "fatal", error = %e, "file synchronization failed");
                        }
                        ErrorCategory::Transient => {
                            error!(operation = "job.file_sync_failed", job_id = %job_id, path = %file_name, category = "transient", error = %e, "file synchronization failed");
                        }
                        _ => {
                            error!(
    ...
    info!(
        ...
        failed = failed_count,
        "concurrent file synchronization completed"
    );

    let quality = adaptive_controller.assess_network_quality().await;
    concurrency_controller.adjust_concurrency(quality);
    info!(
        operation = "job.concurrency_adjusted",
        job_id = %job_id,
        streams = concurrency_controller.get_concurrency(),
        "job concurrency adjusted"
    );

...
use crate::application::sync::engine::{core::FileChunk, ChunkLocation};
...
async fn read_local_chunk(
    ...
    if let Some(pm) = prefetch_manager {
        ...
        crate::shared::file_ops::read_file_range(dest_path, local_offset, local_length as u64)
            .await
            .ok()
    }
}
...
pub async fn verify_strong_hashes(
    ...
    info!(
        operation = "dedup.local_strong_hash_started",
        path = %local_path,
        weak_match_count = weak_matched_indices.len(),
        "local strong hash calculation started"
    );
    ...
    if dedup_count > 0 {
        info!(
            operation = "dedup.completed",
            path = %local_path,
            dedup_chunks = dedup_count,
            total_chunks = chunks.len(),
            dedup_ratio = dedup_count as f64 / chunks.len() as f64,
...
async fn verify_with_remote(
    ...
    info!(
        operation = "dedup.remote_strong_hash_started",
        path = %file.path.display(),
        chunk_count = chunks_to_verify.len(),
        "remote strong hash verification started"
    );

    let chunk_locations: Vec<crate::domain::ChunkLocation> = chunks_to_verify
        .iter()
        .map(|cv| {
            let source_chunk = &chunks[cv.chunk_idx];
            crate::domain::ChunkLocation {
    ...
    {
        Ok(response) => {
            info!(operation = "dedup.remote_strong_hash_received", path = %remote_file_path, hash_count = response.hashes.len(), "remote strong hashes received");

            if response.hashes.len() != chunks_to_verify.len() {
                info!(operation = "dedup.remote_hash_count_mismatch", path = %remote_file_path, received = response.hashes.len(), expected = chunks_to_verify.len(), reason = "verification_failed", "remote strong hash count mismatch");
                return Ok(());
            }
                ...
                chunks_to_verify.iter().zip(response.hashes.iter())
            {
                    ...
                    .find(|candidate| candidate.local_strong == remote_strong)
                {
                    ...
                    *dedup_count += 1;
                    ...
                    info!(operation = "dedup.weak_hash_collision", path = %remote_file_path, chunk_index = verify_info.chunk_idx, candidate_count = verify_info.candidates.len(), "weak hash collision detected");
                }
            }

            info!(operation = "dedup.remote_verification_completed", path = %remote_file_path, verified_count, candidate_count = chunks_to_verify.len(), "remote dedup verification completed");
        }
        Err(e) => {
            info!(operation = "dedup.remote_strong_hash_failed", path = %remote_file_path, error = %e, fallback = "no_dedup", "remote strong hash verification failed");
        }
    }
...
pub fn update_cache(
    cache: &crate::application::sync::engine::CDCResultCache,
    dest_path: &str,
    local_chunks: Vec<crate::shared::cdc::ChunkEntry>,
    ...
    let updated_chunks: Vec<crate::shared::cdc::ChunkEntry> = local_chunks
        .into_iter()
        .map(|mut chunk| {
    ...
    let cache_chunks: Vec<crate::application::sync::engine::ChunkInfo> = updated_chunks
        .iter()
    ...
    if let Err(e) = cache.put(dest_path, file_cache) {
        info!(operation = "dedup.cdc_cache_update_failed", path = %dest_path, error = %e, "CDC cache strong hash update failed");
    } else {
        info!(operation = "dedup.cdc_cache_updated", path = %dest_path, strong_hash_count = computed_strong_hashes.len(), "CDC cache strong hashes updated");
    }
}

...
use hardata_shared::compression::decompress_with_algorithm;
use hardata_shared::error::Result;
use quinn::Connection;
use tracing::{debug, warn};

use super::types::{BatchTransferItem, BatchTransferResult, CancelCallback, ProgressCallback};

use hardata_shared::constants::MAX_ITEMS_PER_REQUEST;
    ...
    pub async fn read_and_write_batch_concurrent_with_progress(
                ...
                (
                    ...
                    tokio::spawn(async move {
                        ...
                        let (mut send, mut recv) = match stream_result {
                            Ok(streams) => streams,
                            Err(e) => {
                                warn!(operation = "transport.batch_stream_open_failed", protocol = "quic", group_index = group_idx, error = %e, "batch stream open failed");
                        ...
                        for batch in group_items.chunks(MAX_ITEMS_PER_REQUEST) {
                            ...
                            .await
                            {
                                ...
                                Err(e) => {
                                    warn!(operation = "transport.batch_failed", protocol = "quic", group_index = group_idx, error = %e, "batch processing failed");
                        ...
                        if let Err(e) = send.finish() {
                            warn!(operation = "transport.batch_stream_finish_failed", protocol = "quic", group_index = group_idx, error = %e, "batch stream finish failed");
                        }
        ...
        for (fallback_failed_indices, task) in tasks {
            match task.await {
                ...
                Err(e) => {
                    warn!(operation = "transport.batch_task_failed", protocol = "quic", error = %e, "batch task failed");
...
async fn process_batch_on_stream(
    ...
    for result in response.results {
        ...
        if idx >= items.len() {
            warn!(
                operation = "transport.batch_result_invalid",
                protocol = "quic",
                item_index = idx,
                item_count = items.len(),
                "batch result index invalid"
            );
            continue;
        }
        ...
        if handled[idx] {
            warn!(
                operation = "transport.batch_result_duplicate",
                protocol = "quic",
                item_index = idx,
                "duplicate batch result index"
            );
            continue;
        }
        handled[idx] = true;
        ...
        if !result.success {
            outcome.failed_indices.push(*global_idx);
            warn!(
                operation = "transport.batch_item_read_failed",
                protocol = "quic",
                item_index = idx,
                error = %result.error.unwrap_or_else(|| "Unknown error".to_string()),
                "batch item read failed"
            );
            continue;
        }
            ...
            match decompress_with_algorithm(&result.data, &compression_info.algorithm) {
                Ok(decompressed) => {
                    if decompressed.len() as u64 != compression_info.original_size {
                        warn!(
                            operation = "transport.batch_decompression_size_mismatch",
                            protocol = "quic",
                            item_index = idx,
                            expected_bytes = compression_info.original_size,
                            actual_bytes = decompressed.len(),
                            "batch decompressed size mismatch"
                        );
                        outcome.failed_indices.push(*global_idx);
                        continue;
                    }
                ...
                Err(e) => {
                    warn!(operation = "transport.batch_decompression_failed", protocol = "quic", item_index = idx, error = %e, "batch decompression failed");
                    outcome.failed_indices.push(*global_idx);
                    continue;
                }
            }
        ...
        match hardata_shared::file_ops::write_file_range(&item.dest_path, item.dest_offset, &data)
            .await
        {
            Ok(_) => {
            ...
            Err(e) => {
                outcome.failed_indices.push(*global_idx);
                warn!(operation = "transport.batch_item_write_failed", protocol = "quic", item_index = idx, error = %e, "batch destination write failed");
            }
        }
    }
...
mod tests {
    ...
    use crate::transfer::batch::BatchTransferItem;

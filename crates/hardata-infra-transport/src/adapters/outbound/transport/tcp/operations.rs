...
use hardata_shared::time::unix_timestamp_millis;
...
impl TcpClient {
    pub async fn get_file_hashes(
    ...
    ) -> Result<hardata_protocol::GetFileHashesResponse> {
        debug!("Get file hashes: {}", file_path);

        let request = hardata_protocol::GetFileHashesRequest {
        ...
        let response: hardata_protocol::GetFileHashesResponse = bincode::deserialize(&payload_buf)
            .map_err(|e| {
                HarDataError::SerializationError(format!(
    ...
    pub async fn get_strong_hashes(
        ...
        stream: &mut TcpStream,
        file_path: &str,
        chunks: Vec<hardata_protocol::ChunkLocation>,
    ) -> Result<hardata_protocol::GetStrongHashesResponse> {
        debug!(
        ...
        let request = hardata_protocol::GetStrongHashesRequest {
            file_path: file_path.to_string(),
            chunks,
        };
        ...
        let response: hardata_protocol::GetStrongHashesResponse =
    ...
    pub async fn list_directory(
        ...
        stream: &mut TcpStream,
        directory_path: &str,
    ) -> Result<hardata_protocol::ListDirectoryResponse> {
        debug!("List directory: {}", directory_path);

        let request = hardata_protocol::ListDirectoryRequest {
        ...
        let response: hardata_protocol::ListDirectoryResponse = bincode::deserialize(&payload_buf)
            .map_err(|e| {
                HarDataError::SerializationError(format!(
    ...
    async fn ping_stream(&self, stream: &mut TcpStream) -> Result<u64> {
        ...
        let request = hardata_protocol::PingRequest { timestamp };
        let payload = bincode::serialize(&request).map_err(|e| {
        ...
        let response: hardata_protocol::PongResponse =
            bincode::deserialize(&payload_buf).map_err(|e| {
                HarDataError::SerializationError(format!(
...
mod tests {
    use super::TcpClient;
    use hardata_protocol::{GetStrongHashesRequest, ISyncMessage, MessageType};
    use tokio::io::{AsyncReadExt, AsyncWriteExt};

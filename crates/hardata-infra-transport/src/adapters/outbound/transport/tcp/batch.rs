...
async fn process_group(
    ...
    let mut conn = match client.get_pooled_connection().await {
        ...
        Err(e) => {
            warn!(
                operation = "transport.batch_connection_failed",
                protocol = "tcp",
                group_index = group_idx,
                error = %e,
                "batch connection acquisition failed"
            );
    ...
    for batch in group_items.chunks(MAX_ITEMS_PER_REQUEST) {
        match process_batch(&mut conn, batch, &progress_callback, &cancel_callback).await {
            ...
            Err(e) => {
                warn!(operation = "transport.batch_failed", protocol = "tcp", group_index = group_idx, error = %e, "batch processing failed");
...
impl TcpClient {
    pub async fn read_and_write_batch_concurrent(
        ...
        job_id: &str,
        concurrency: usize,
    ) -> hardata_shared::error::Result<BatchTransferResult> {
    ...
    pub async fn read_and_write_batch_concurrent_with_progress(
        ...
        for (fallback_failed_indices, task) in tasks {
            match task.await {
                ...
                Err(e) => {
                    warn!(operation = "transport.batch_task_failed", protocol = "tcp", error = %e, "batch task failed");
...
async fn process_batch(
    ...
    let response: hardata_protocol::ReadBlockResponse = bincode::deserialize(&payload)
    ...
    for result in response.results {
        ...
        let data = if let Some(compression_info) = &result.compression {
            match decompress_with_algorithm(&result.data, &compression_info.algorithm) {
                ...
                Err(e) => {
                    warn!(operation = "transport.batch_decompression_failed", protocol = "tcp", item_index = idx, error = %e, "batch decompression failed");
                    outcome.failed_indices.push(*global_idx);
                    continue;
                }
            }
            ...
            .await
        {
            ...
            Err(e) => {
                outcome.failed_indices.push(*global_idx);
                warn!(operation = "transport.batch_item_write_failed", protocol = "tcp", item_index = idx, error = %e, "batch destination write failed");
            }
        }
    }

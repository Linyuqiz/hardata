...
use hardata_shared::file_ops::read_exact_at;
use rayon::prelude::*;
use std::fs::File;
use std::path::Path;
use tracing::info;
...
impl ComputeService {
    pub async fn get_strong_hashes(
        ...
        chunks: &[hardata_protocol::ChunkLocation],
    ) -> Result<Vec<hardata_protocol::StrongHashResult>> {
        ...
        let results = tokio::task::spawn_blocking(move || {
            ...
            let results: Vec<hardata_protocol::StrongHashResult> = validated_chunks
                .par_iter()
                .map(
                    |(offset, length)| -> std::io::Result<hardata_protocol::StrongHashResult> {
                        let mut data = vec![0u8; *length];
                        read_exact_at(&file, *offset, &mut data)?;
                        let strong_hash = *blake3::hash(&data).as_bytes();
                        Ok(hardata_protocol::StrongHashResult {
                            offset: *offset,
                            strong_hash,
                        })
                    },
        ...
        info!(
            operation = "agent.strong_hashes_completed",
            path = %target_file_path.display(),
            hash_count = results.len(),
            "strong hash calculation completed"
        );
...
mod tests {
    ...
    use hardata_protocol::ChunkLocation;

...
use hardata_shared::time::{metadata_ctime_nanos, metadata_inode, metadata_mtime_nanos};
use std::path::Path;
...
impl ComputeService {
    pub async fn get_file_hashes(
    ...
    ) -> Result<(u64, Arc<Vec<hardata_protocol::ChunkMetadata>>)> {
        use hardata_shared::cdc::{StreamingFastCDC, StreamingFastCDCConfig};
        ...
        info!(
            operation = "agent.file_chunk_cache_miss",
            path = %relative_path,
            min_chunk_size,
            avg_chunk_size,
            max_chunk_size,
            "file chunk cache miss"
        );
        ...
        let chunks: Vec<hardata_protocol::ChunkMetadata> = chunk_entries
            .into_iter()
            .map(|entry| hardata_protocol::ChunkMetadata {
                offset: entry.offset,
    ...
    pub async fn list_directory(
        &self,
        directory_path: &Path,
    ) -> Result<Vec<hardata_protocol::FileInfo>> {
        use tokio::fs;
        ...
        if root_symlink_metadata.file_type().is_symlink() {
            ...
            info!(operation = "agent.root_symlink_detected", path = %directory_path.display(), "root symlink detected");
            return Ok(vec![hardata_protocol::FileInfo {
                path: file_name,
        ...
        if directory_path.is_file() {
            ...
            info!(operation = "agent.single_file_detected", path = %directory_path.display(), "single file path detected");
            return Ok(vec![hardata_protocol::FileInfo {
                path: file_name,
                size: metadata.len(),
                is_directory: false,
            ...
            .map_err(|e| HarDataError::FileOperation(format!("Failed to read entry: {}", e)))?
        {
            ...
            files.push(hardata_protocol::FileInfo {
                path: relative_path,
                size: metadata.len(),
        ...
        info!(
            operation = "agent.directory_listed",
    ...
    pub async fn read_block_by_offset(
        ...
        use hardata_shared::handle_pool::acquire_file_handle;

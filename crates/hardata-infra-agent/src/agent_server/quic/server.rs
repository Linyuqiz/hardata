            ...
            match self.endpoint.accept().await {
                Some(incoming) => {
                    let permit = match self.connection_semaphore.clone().try_acquire_owned() {
                        ...
                        Err(_) => {
                            warn!(
                                operation = "agent.quic_connection_rejected",
                                max_connections = MAX_CONCURRENT_CONNECTIONS,
                                "QUIC connection rejected at capacity"
                            );
                            continue;
                        }
                    };
                    ...
                    tokio::spawn(async move {
                        match incoming.await {
                            Ok(connection) => {
                                ...
                                .await
                                {
                                    error!(operation = "agent.quic_connection_failed", peer = %peer, error = %e, "QUIC connection failed");
                                }
                            }
                            Err(e) => {
                                error!(operation = "agent.quic_handshake_failed", error = %e, "QUIC handshake failed");
                            }
                        }
                ...
                None => {
                    warn!(
                        operation = "agent.quic_accept_stopped",
                        "QUIC server stopped accepting connections"
                    );
                    break;
                }
            }
...
async fn handle_connection(
    ...
    debug!(operation = "agent.quic_connection_accepted", peer = %connection.remote_address(), "QUIC connection accepted");
        ...
        match connection.accept_bi().await {
            Ok((send, recv)) => {
                let permit = match stream_semaphore.clone().try_acquire_owned() {
                    ...
                    Err(_) => {
                        warn!(
                            operation = "agent.quic_stream_rejected",
                            peer = %connection.remote_address(),
                            max_streams = MAX_CONCURRENT_STREAMS,
                            "QUIC stream rejected at capacity"
                        );
                        continue;
                    }
                };
                ...
                tokio::spawn(async move {
                    if let Err(e) = handle_stream(send, recv, compute, data_dir).await {
                        error!(operation = "agent.quic_stream_failed", error = %e, "QUIC stream failed");
                    }
                    drop(permit);
                });
            }
            Err(e) => {
                debug!(operation = "agent.quic_connection_closed", peer = %connection.remote_address(), error = %e, "QUIC connection closed");
                break;
            }
        }
    }
...
async fn handle_stream(
    ...
    loop {
        ...
        let header_read_result = match timeout(request_timeout, recv.read_exact(&mut header)).await
        {
            ...
            Err(_) => {
                warn!(
                    operation = "agent.quic_request_timeout",
                    timeout_secs = REQUEST_TIMEOUT_SECS,
                    "QUIC request timed out"
                );
        ...
        match header_read_result {
            ...
            Err(e) => {
                tracing::error!(operation = "agent.quic_header_read_failed", error = %e, "QUIC request header read failed");
        ...
        if payload_len > MAX_PAYLOAD_SIZE {
            error!(
                operation = "agent.quic_payload_rejected",
                payload_len,
                max_payload_size = MAX_PAYLOAD_SIZE,
                "QUIC payload exceeds configured limit"
            );
        ...
        match msg_type {
            ...
            _ => {
                warn!(operation = "agent.quic_message_unsupported", message_type = ?msg_type, "unsupported QUIC message type");

...
pub use crate::agent_server::common::{
...
fn tls_paths(data_dir: &Path, primary_hostname: &str) -> (PathBuf, PathBuf, PathBuf, PathBuf) {
    ...
    // Tests use isolated TLS material; production keeps the historical path.
    #[cfg(test)]
...
pub fn load_or_generate_self_signed_cert(
        ...
        let should_regenerate = match metadata_hostnames.as_ref() {
            ...
            None => match load_hostnames_from_certificate(&cert_der) {
                ...
                Ok(existing_hostnames) => {
                    warn!(
                        operation = "agent.quic_certificate_rotated",
                        path = %tls_dir.display(),
                        reason = "hostname_mismatch",
                        existing_hostname_count = existing_hostnames.len(),
                        desired_hostname_count = subject_alt_names.len(),
                        "QUIC certificate hostname set changed"
                    );
                    true
                }
                Err(e) => {
                    warn!(
                        operation = "agent.quic_certificate_metadata_invalid",
                        path = %tls_dir.display(),
                        error = %e,
                        fallback = "reuse_existing_certificate",
                        "QUIC certificate metadata could not be parsed"
                    );
                    return Ok((certs, key));
                }
            },
    ...
    if !has_complete_material && (cert_path.exists() || key_path.exists()) {
        warn!(
            operation = "agent.quic_certificate_rotated",
            path = %tls_dir.display(),
            reason = "incomplete_tls_material",
            "incomplete QUIC TLS material replaced"
        );
    }

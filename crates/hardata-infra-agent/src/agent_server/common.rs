...
pub const MAX_PAYLOAD_SIZE: usize = hardata_shared::constants::MAX_PROTOCOL_PAYLOAD_SIZE;
...
pub fn validate_request_item_count(operation: &str, count: usize) -> Result<()> {
    let max = hardata_shared::constants::MAX_ITEMS_PER_REQUEST;
    if count > max {
...
pub fn compress_block_data(
    target_path: &Path,
    data: Vec<u8>,
) -> (Vec<u8>, Option<hardata_protocol::CompressionInfo>) {
    use hardata_shared::compression::{smart_compress, CompressionLevel, FileTypeDetector};
    ...
    match smart_compress(&data, target_path, CompressionLevel::Fastest) {
        Ok((compressed, algorithm)) => {
            ...
            if algorithm == "none" {
                ...
                (
                    compressed,
                    Some(hardata_protocol::CompressionInfo {
                        algorithm,
                        original_size,
                        compressed_size,
                    }),
        ...
        Err(e) => {
            warn!(
                operation = "agent.compression_failed",
                path = %target_path.display(),
                error = %e,
                "agent response compression failed"
            );
            (data, None)
        }
    }

    ...
    value
        .map(|raw| {
            DateTime::parse_from_rfc3339(&raw)
                ...
                .map_err(|e| {
                    hardata_shared::error::HarDataError::SerializationError(format!(
                        "Invalid {}: {}",
                        field, e
                    ))
                })
...
impl Database {
    ...
    pub async fn update_retry_attempt(&self, job_id: &str, success: bool) -> Result<bool> {
        ...
        if retry.is_none() {
            warn!(operation = "retry.record_missing", job_id = %job_id, "retry record not found");
            return Ok(false);
        }
        ...
        if new_count >= retry.max_retries {
            ...
            .execute(&self.pool)
            .await?;
            info!(operation = "retry.exhausted", job_id = %job_id, "retry limit reached");
            return Ok(true);
        }
        ...
        info!(
            operation = "retry.attempt_recorded",
            job_id = %job_id,
    ...
    /// Restore a retry record from a finalize rollback snapshot.
